...
struct FSInfo read_FSInfo(HANDLE current_device_descriptor)
{
	struct FSInfo current_FSInfo;
	if (ReadFile(current_device_descriptor, &current_FSInfo, sizeof(struct FSInfo), NULL, NULL) == 0) error_read_device();
