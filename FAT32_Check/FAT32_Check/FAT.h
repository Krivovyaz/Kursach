...
char** read_FAT(HANDLE device_descriptor, __int8 offset_first_FAT, __int2 mirror_flags, __int1 number_FAT_copies, __int8 count_bytes_FAT)
{
	int current_number_FAT = number_FAT_copies;
	int current_flag = (mirror_flags & 0x0080);
	int is_simple_FAT = 0;
	...
	seek_descriptor(device_descriptor, offset_first_FAT);
